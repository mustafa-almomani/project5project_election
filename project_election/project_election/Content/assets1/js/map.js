...
provinces.forEach(province => {
    ...
    div.addEventListener('click', function(event) {
        ...
        showPopup(province.name, province.info, event.clientX, event.clientY);
    });
...
// عرض Popup
function showPopup(provinceName, provinceInfo, x, y) {
    const popup = document.getElementById('popup');
    popup.innerHTML = `<h2>${provinceName}</h2>
                        <p>${provinceInfo}</p>
                        <button onclick="closePopup()">إغلاق</button>`;
    popup.style.left = `${x + 10}px`; // موضع الـ Popup على محور X
    popup.style.top = `${y + 10}px`; // موضع الـ Popup على محور Y
